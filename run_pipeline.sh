...
#   dataset=germany_val,germany_test \
...
python graide/run_screening.py -m \
  ...
  dataset=sweden_val,sweden_test \
  screening_system=GERMANY  # because the process is the same

# Generate artefacts (unfortunately can't multi-run over config names) -- does this create the artefacts for all three screening names? Or just the last run?
